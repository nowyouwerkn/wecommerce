<table class="table table-dashboard table-responsive">
    ...
    <tbody>
        @foreach ($orders as $order)
            <tr>
                ...
                <td class="text-muted">
                    <span style="display:block; width:100px;" data-toggle="tooltip" data-placement="bottom"
                ...
                <td>
                    @if ($order->user == null)
                        <span class="badge badge-warning"><i class="fas fa-user-alt-slash"></i> Eliminado por
                            Administración</span>
                    @else
                        {{ $order->user->name ?? '' }}
                    @endif
                ...
                <td>
                    <span class="text-muted"><i class="far fa-clock"></i>
                        {{ Carbon\Carbon::parse($order->created_at)->translatedFormat('d M Y - h:ia') }}</span>
                </td>

                <td><strong>${{ number_format($order->payment_total, 2) }}</strong><i
                        class="far fa-question-circle ml-1" style="font-size:.7em;" data-toggle="tooltip"
                    ...
                    @switch($order->status)
                        ...
                        @case('Pagado')
                            <div class="alert alert-primary d-flex" role="alert">
                                ...
                                <div>
                                    <strong>Excelente</strong> tu cliente ya pagó. Ahora empaqueta el producto. Nosotros le
                                    mandaremos automáticamente un correo con su estado.
                                </div>
                            </div>
                        @break
                        ...
                        @case('Empaquetado')
                            <div class="alert alert-warning d-flex" role="alert">
                                ...
                                <div>
                                    <strong>Asegurate de enviar el paquete</strong>. Genera una guía para la orden e imprime la
                                    lista de empaque <a href="{{ route('order.packing.list', $order->id) }}">desde aquí.</a>
                                </div>
                            </div>
                        @break
                        ...
                        @case('Enviado')
                            <div class="alert alert-success d-flex" role="alert">
                                ...
                                <div>
                                    <strong>¡Perfecto!</strong> Asegurate de actualizar a "Entregado" en cuanto sepas que tu
                                    cliente recibió su paquete.
                                </div>
                            </div>
                        @break
                        ...
                        @case('Expirado')
                            <div class="alert alert-secondary d-flex" role="alert">
                                <i class="fas fa-truck mr-3" style="position:relative; top:3px;"></i>
                                <div>
                                    <strong>¡No te desanimes!</strong> Algunas ordenes no se pueden completar por falta de
                                    fondos de tu cliente o tuvo problemas con su tarjeta. Le mandaremos un correo más adelante
                                    para invitarlo al sitio nuevamente.
                                </div>
                            </div>
                        @break
                        ...
                        @case('Reembolsado')
                            <div class="alert alert-secondary d-flex" role="alert">
                                <i class="fas fa-truck mr-3" style="position:relative; top:3px;"></i>
                                <div>
                                    <strong>¡No te desanimes!</strong> Pregunta a tu cliente el motivo y que soluciones puede
                                    haber para mejorar el servicio/producto.
                                </div>
                            </div>
                        @break
                ...
                <td>
                    @if ($order->type != 'recurring_payment')
                        <div id="orderStatus_{{ $order->id }}" class="dropdown order-status">
                            ...
                            <button class="badge badge-table btn btn-{{ Str::slug($order->status) }} dropdown-toggle"
                            ...
                            <div class="dropdown-menu">
                                @if ($order->status == 'Cancelado' || $order->status == 'Expirado' || $order->status == 'Sin Completar')
                                    <p class="mb-0 mt-2 pl-3"><i class="fas fa-times"></i></span> Orden
                                        {{ $order->status }}</p>
                                @else
                                    <h6 class="dropdown-header tx-uppercase tx-12 tx-bold tx-inverse">Cambiar Estatus
                                    </h6>
                                    <a class="
                            dropdown-item
                            @if ($order->status == 'Pagado' ||
                                ...
                                $order->status == 'Entregado') disabled @endif
                            "
                                ...
                                <a class="
                            dropdown-item
                            @if ($order->status == 'Empaquetado' || $order->status == 'Enviado' || $order->status == 'Entregado') disabled @endif
                            "
                    ...
                    @if ($order->shipping_option == null or $order->shipment == null)
                        <a class="
                                dropdown-item
                                @if ($order->status == 'Enviado' || $order->status == 'Entregado') disabled @endif
                                "
        ...
        <a class="
                            dropdown-item
                            @if ($order->status == 'Entregado') disabled @endif
                            "
        ...
        @if ($order->status != 'Entregado')
            ...
            <a class="dropdown-item text-danger" href="#" data-value="Cancelado"><span class="step-style"><i
                        class="fas fa-times"></i></span> Cancelar Orden</a>
        @endif
        @endif
        </div>
        </div>
                ...
                $('#orderStatus_{{ $order->id }} .dropdown-item').on('click', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(msg) {
                            ...
                            switch (status) {
                                case 'pendiente':
                                    $('#orderStatus_{{ $order->id }} .order-info i').replaceWith(
                                        '<i class="fas fa-exclamation mr-1"></i>');
                                    break;
                                case 'pagado':
                                    $('#orderStatus_{{ $order->id }} .order-info i').replaceWith(
    ...
    @else
        ...
        <td>
            @if ($order->status == 'Empaquetado' || $order->status == 'Enviado' || $order->status == 'Entregado')
                <a data-toggle="tooltip" data-placement="top" title="Imprimir lista de empaque"
                    href="{{ route('order.packing.list', $order->id) }}">
